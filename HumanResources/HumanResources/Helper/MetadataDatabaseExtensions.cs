        ...
        public static bool AddOrUpdate(this MetadataDatabase mdd, Tag_MDD tag)
        {
			...
			if (tag_MDD == null)
			{
				try
				{
					...
					Mod.Log.Debug?.Write($"Inserted tag: {tag.Name}");
					success = true;
				}
				catch (Exception e)
				{
					Mod.Log.Error?.Write(e, $"Failed to insert tag: {tag.Name} into MDD");
					return false;
				}
			}
			else
            {
				try
                {
					...
					Mod.Log.Debug?.Write($"Updated tag: {tag.Name}");
					success = true;
				}	
				catch (Exception e)
                {
					Mod.Log.Error?.Write(e, $"Failed to update tag: {tag.Name} in MDD");
					return false;
				}
			}
